...
This document describes the target architecture for first-class remote environments in Snocode.
...
- Keep the Snocode server as the execution boundary.
...
Snocode already has a clean runtime boundary: the client talks to a Snocode server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Snocode server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Snocode server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Snocode runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Snocode server instance.
...
How does the client speak WebSocket to a Snocode server?
...
- best fit for hosted or self-managed remote Snocode deployments
...
For Snocode, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Snocode-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Snocode server
...
How does a Snocode server come to exist on the target machine?
...
The user or operator already runs Snocode on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Snocode:
...
The recommended Snocode flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Snocode availability.
3. Desktop launches or reuses a remote Snocode server.
...
This is the inverse of remote launch: a local Snocode server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Snocode server might be reached through direct `wss`.
...
Snocode already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Snocode should not copy that part.

Snocode already has the right runtime boundary:

- one Snocode server per environment
- ordinary HTTP/WebSocket between client and environment

So Snocode should borrow Zed's launch discipline, not its transport protocol.
