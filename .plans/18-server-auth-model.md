...
Define the long-term server auth architecture for Snocode before first-class remote environments ship.
...
Every privileged surface of the Snocode server must go through the same auth policy engine:
...
```text
Participants:
  UserShell     = npx snocode launcher
...
DesktopMain -> Tunnel : publish local Snocode endpoint
...
Participants:
  ...
  RemoteT3      = remote Snocode server
  Frontend      = desktop renderer
...
SSH -> RemoteHost : snocode remote launch --json
