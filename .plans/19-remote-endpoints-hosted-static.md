...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later Snocode Tunnel
- users can generate a hosted pairing link for `app.snowcode.io`
...
This plan intentionally leaves the paid Snocode cloud tunnel fabric out of scope. It defines the OSS foundation that Snocode Tunnel should later plug into.
...
- `https://app.snowcode.io` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.snowcode.io` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.snowcode.io/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.snowcode.io`.
...
- `app.snowcode.io` works as a real client shell.
...
- `app.snowcode.io` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.snowcode.io`.
...
## Phase 4: Future Snocode Tunnel Provider
...
The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of Snocode should only see an `AdvertisedEndpoint`.
