      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(input.legacyPathExists === true && path.includes("Snocode (Alpha)")),
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"snocodeCommitHash":"abcdef1234567890"}'),
          }),
        ),
  ...
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Snocode (Alpha)");
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Snocode (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Snocode (Alpha)");
