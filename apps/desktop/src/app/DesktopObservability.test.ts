...
const testHttpClientLayer = Layer.succeed(
  ...
  HttpClient.make((request) =>
    Effect.succeed(HttpClientResponse.fromWeb(request, new Response("", { status: 204 }))),
  ),
);
...
describe("DesktopObservability", () => {
  it.effect("persists desktop Effect logs as span events in desktop.trace.ndjson", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-desktop-observability-test-",
      });
    ...
    }).pipe(Effect.scoped, Effect.provide(Layer.mergeAll(NodeServices.layer, testHttpClientLayer))),
  );
  ...
  it.effect("persists backend child output as structured JSON records in development", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-desktop-backend-output-log-test-",
      });
      const environmentLayer = makeEnvironmentLayer(baseDir);
      const logPath = yield* Effect.gen(function* () {
        const environment = yield* DesktopEnvironment.DesktopEnvironment;
