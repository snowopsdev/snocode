...
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@snocode/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ SNOCODE_HOME: baseDir })),
    ),
  );
...
const withSavedEnvironments = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "snocode-desktop-saved-environments-test-",
    });
