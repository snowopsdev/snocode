...
import { SshHttpBridgeError } from "@snocode/ssh/errors";
import { fetchLoopbackSshJson } from "@snocode/ssh/tunnel";
  ...
  return DesktopSshRemoteApi.of({
    fetchEnvironmentDescriptor: ({ httpBaseUrl }) =>
      fetchLoopbackSshJson<unknown>({
        httpBaseUrl,
        pathname: "/.well-known/snocode/environment",
      }).pipe(
