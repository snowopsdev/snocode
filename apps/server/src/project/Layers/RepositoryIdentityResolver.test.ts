...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-repository-identity-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:snowopsdev/snocode.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/snowopsdev/snocode");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("snowopsdev/snocode");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("snowopsdev");
      expect(identity?.name).toBe("snocode");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      const repoRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-repository-identity-nested-root-test-",
      });
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:snowopsdev/snocode.git"]);
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-repository-identity-no-remote-",
      });
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-repository-identity-upstream-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/snocode.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:snowopsdev/snocode.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-repository-identity-nested-group-test-",
      });
      ...
      yield* git(cwd, [
        ...
        "add",
        "origin",
        "git@gitlab.com:snowopsdev/platform/snocode.git",
      ]);
      ...
      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/snowopsdev/platform/snocode");
      expect(identity?.displayName).toBe("snowopsdev/platform/snocode");
      expect(identity?.owner).toBe("snowopsdev");
      expect(identity?.name).toBe("snocode");
    }).pipe(Effect.provide(RepositoryIdentityResolverLive)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "snocode-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/snowopsdev/snocode");
        expect(refreshedIdentity?.name).toBe("snocode");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/snowopsdev/snocode");

      yield* git(cwd, [
        "remote",
        "set-url",
        "origin",
        "git@github.com:snowopsdev/snocode-next.git",
      ]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/snowopsdev/snocode");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/snowopsdev/snocode-next");
      expect(refreshedIdentity?.displayName).toBe("snowopsdev/snocode-next");
      expect(refreshedIdentity?.name).toBe("snocode-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
