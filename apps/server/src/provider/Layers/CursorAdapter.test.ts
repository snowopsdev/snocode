...
export SNOCODE_ACP_REQUEST_LOG_PATH=${JSON.stringify(requestLogPath)}
${envExports}
exec ${JSON.stringify(bunExe)} ${JSON.stringify(mockAgentPath)} "$@"
`;
  await writeFile(wrapperPath, script, "utf8");
  ...
  Layer.effect(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "snocode-cursor-adapter-test-",
      }),
    ),
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
    ...
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({
          SNOCODE_ACP_EXIT_LOG_PATH: exitLogPath,
        }),
      );
      yield* settings.updateSettings({ providers: { cursor: { binaryPath: wrapperPath } } });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const previousEmitToolCalls = process.env.SNOCODE_ACP_EMIT_TOOL_CALLS;
        process.env.SNOCODE_ACP_EMIT_TOOL_CALLS = "1";
        ...
        const wrapperPath = yield* Effect.promise(() =>
          makeMockAgentWrapper({ SNOCODE_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
          ...
          Effect.ensuring(
            Effect.sync(() => {
              if (previousEmitToolCalls === undefined) {
                delete process.env.SNOCODE_ACP_EMIT_TOOL_CALLS;
              } else {
                process.env.SNOCODE_ACP_EMIT_TOOL_CALLS = previousEmitToolCalls;
              }
            }),
          ),
        );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => writeFile(requestLogPath, "", "utf8"));
        const wrapperPath = yield* Effect.promise(() =>
          makeProbeWrapper(requestLogPath, argvLogPath, { SNOCODE_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
  ...
  it.effect("segments assistant messages around ACP tool activity in full-access mode", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ SNOCODE_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS: "1" }),
      );
      yield* serverSettings.updateSettings({
        providers: { cursor: { binaryPath: wrapperPath } },
      });
  ...
  it.effect("stopping a session settles pending user-input waits", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ SNOCODE_ACP_EMIT_ASK_QUESTION: "1" }),
      );
      yield* serverSettings.updateSettings({ providers: { cursor: { binaryPath: wrapperPath } } });
  ...
  it.effect(
    ...
    () => {
      ...
      const customAdapterLayer = Layer.effect(
        ...
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), {
            prefix: "snocode-cursor-adapter-custom-instance-",
          }),
        ),
        Layer.provideMerge(NodeServices.layer),
      );
