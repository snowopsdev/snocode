  ...
  Effect.gen(function* () {
    ...
    const runtime = ManagedRuntime.make(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "snocode-web",
          attributes: {
            "service.runtime": "snocode-web",
...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const config: ServerConfigShape = {
      ...
      otlpServiceName: "snocode-server",
      mode: "desktop",
      port: 0,
      host: "127.0.0.1",
      cwd: process.cwd(),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves project favicon requests before the dev URL redirect", () =>
    Effect.gen(function* () {
      ...
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-router-project-favicon-",
      });
  ...
  it.effect("serves the fallback project favicon when no icon exists", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "snocode-router-project-favicon-fallback-",
      });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/snocode/environment");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const projectDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "snocode-router-project-favicon-query-token-",
        });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "snocode-web");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "snocode-ws-auth-required-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "snocode-ws-project-search-",
      });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
  ...
  it.effect("routes websocket rpc projects.searchEntries excludes gitignored files", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "snocode-ws-project-search-gitignored-",
      });
  ...
  it.effect("routes websocket rpc projects.listDirectory with ignore filtering", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "t3-ws-project-list-",
      });
      yield* fs.writeFileString(path.join(workspaceDir, ".gitignore"), "generated/\n");
      yield* fs.makeDirectory(path.join(workspaceDir, ".github"));
      yield* fs.makeDirectory(path.join(workspaceDir, "generated"));
      yield* fs.makeDirectory(path.join(workspaceDir, "node_modules"));
      yield* fs.makeDirectory(path.join(workspaceDir, "src"));
      yield* fs.writeFileString(path.join(workspaceDir, "README.md"), "# ok\n");
      ...
      yield* buildAppUnderTest({
        layers: {
          vcsDriver: {
            ...
            filterIgnoredPaths: (_cwd, relativePaths) =>
              Effect.succeed(relativePaths.filter((relativePath) => relativePath !== "generated")),
          },
        },
      });
  ...
  it.effect("routes websocket rpc projects.createFile", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-ws-project-file-" });
      ...
      const response = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[WS_METHODS.projectsCreateFile]({
            cwd: workspaceDir,
            name: "created.txt",
            contents: "written-by-create",
          }),
        ),
      );

      assert.equal(response.relativePath, "created.txt");
      const persisted = yield* fs.readFileString(path.join(workspaceDir, "created.txt"));
      assert.equal(persisted, "written-by-create");
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("routes websocket rpc projects.createFolder", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-ws-project-folder-" });
      ...
      const response = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[WS_METHODS.projectsCreateFolder]({
            cwd: workspaceDir,
            name: "docs",
          }),
        ),
      ...
      assert.equal(response.relativePath, "docs");
      const stat = yield* fs.stat(path.join(workspaceDir, "docs"));
      assert.equal(stat.type, "Directory");
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("routes websocket rpc projects.createFile conflict errors", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-ws-project-file-" });
      yield* fs.writeFileString(path.join(workspaceDir, "README.md"), "# existing\n");
      ...
      assertTrue(result._tag === "Failure");
      assertTrue(result.failure._tag === "ProjectCreateFileError");
      assert.equal(result.failure.code, "already-exists");
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("routes websocket rpc projects.createFile path traversal errors", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-ws-project-file-" });
      ...
      assertTrue(result._tag === "Failure");
      assertTrue(result.failure._tag === "ProjectCreateFileError");
      assert.equal(result.failure.code, "outside-root");
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "snocode-ws-project-write-",
      });
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const parentDir = yield* fs.makeTempDirectoryScoped({ prefix: "snocode-ws-project-create-" });
  ...
  it.effect("enriches replayed project events with repository identity metadata", () =>
    Effect.gen(function* () {
      const repositoryIdentity = {
        canonicalKey: "github.com/snowopsdev/snocode",
        locator: {
          source: "git-remote" as const,
          remoteName: "origin",
          remoteUrl: "git@github.com:snowopsdev/snocode.git",
        },
        displayName: "Snocode/snocode",
        provider: "github",
        owner: "Snocode",
        name: "snocode",
      };
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "snocode/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "snocode/bootstrap-refName",
                },
                runSetupScript: true,
              },
              createdAt,
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          cwd: "/tmp/project",
          refName: "main",
          newRefName: "snocode/bootstrap-refName",
          path: null,
        });
