...
import { sanitizeBranchFragment } from "@snocode/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@snocode/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("SNOCODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("SNOCODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("SNOCODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("SNOCODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
export class BitbucketApi extends Context.Service<BitbucketApi, BitbucketApiShape>()(
  "snocode/source-control/BitbucketApi",
) {}
...
function checkoutBranchName(input: {
  ...
  return `snocode/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set SNOCODE_BITBUCKET_EMAIL and SNOCODE_BITBUCKET_API_TOKEN, or SNOCODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
