...
import { GitCommandError, type SourceControlProviderError } from "@snocode/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/snocode",
  sshUrl: "git@github.com:octocat/snocode.git",
};
...
function makeLayer(input: {
  ...
  return SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), { prefix: "snocode-source-control-repos-" }),
    ),
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/snocode" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "snocode-source-control-clone-parent-",
    });
    const destinationPath = `${parent}/snocode`;
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "snocode"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/snocode", visibility: "private" },
    ]);
