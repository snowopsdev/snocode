...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("SNOCODE_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("SNOCODE_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("SNOCODE_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("SNOCODE_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.number("SNOCODE_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          arch: process.arch,
          snocodeVersion: packageJson.version,
          clientType,
        },
