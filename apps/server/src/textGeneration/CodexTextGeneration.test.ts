...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@snocode/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "snocode-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__SNOCODE_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__SNOCODE_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "snocode-codex-text-" });
