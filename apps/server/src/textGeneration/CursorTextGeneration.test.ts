...
import { CursorSettings, ProviderInstanceId } from "@snocode/contracts";
...
const CursorTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "snocode-cursor-text-generation-test-",
...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "snocode-cursor-text-acp-"));
...
it.layer(CursorTextGenerationTestLayer)("CursorTextGeneration", (it) => {
  it.effect("uses ACP model config options instead of raw CLI model ids", () => {
    const requestLogDir = mkdtempSync(path.join(os.tmpdir(), "snocode-cursor-text-log-"));
    const requestLogPath = path.join(requestLogDir, "requests.ndjson");

    return withFakeAcpAgent(
      {
        SNOCODE_ACP_REQUEST_LOG_PATH: requestLogPath,
        SNOCODE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("accepts json objects with extra assistant text around them", () =>
    withFakeAcpAgent(
      {
        SNOCODE_ACP_PROMPT_RESPONSE_TEXT:
  ...
  it.effect("closes the ACP child process after text generation completes", () => {
    const exitLogDir = mkdtempSync(path.join(os.tmpdir(), "snocode-cursor-text-exit-log-"));
