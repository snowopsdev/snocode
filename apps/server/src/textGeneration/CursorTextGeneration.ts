...
import { type CursorSettings, type ModelSelection } from "@snocode/contracts";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@snocode/shared/git";
import { extractJsonObject } from "@snocode/shared/schemaJson";

import { TextGenerationError } from "@snocode/contracts";
  ...
  const runCursorJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const runtime = yield* makeCursorAcpRuntime({
        ...
        cwd,
        clientInfo: { name: "snocode-git-text", version: "0.0.0" },
      });
