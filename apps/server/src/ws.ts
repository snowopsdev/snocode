...
function workspaceFileSystemErrorDetail(cause: unknown): string | null {
...
function workspaceFileSystemErrorCode(cause: unknown): string | undefined {
...
function workspaceFileOperationMessage(input: {
  readonly action: string;
  readonly unsafePathMessage: string;
  readonly cause: unknown;
}): string {
  if (isWorkspacePathOutsideRootError(input.cause)) {
    return input.unsafePathMessage;
  }
  const detail = workspaceFileSystemErrorDetail(input.cause);
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      return WsRpcGroup.of({
              ...
              Effect.mapError(
                (cause) =>
                  new ProjectListDirectoryError({
                    message: workspaceFileOperationMessage({
                      action: "Failed to list workspace directory",
                      unsafePathMessage:
                        "Workspace directory path must stay within the project root.",
                      cause,
                    }),
                    code: workspaceFileSystemErrorCode(cause),
                    cause,
                  }),
              ),
            ),
              ...
              Effect.mapError(
                (cause) =>
                  new ProjectReadFileError({
                    message: workspaceFileOperationMessage({
                      action: "Failed to read workspace file",
                      unsafePathMessage: "Workspace file path must stay within the project root.",
                      cause,
                    }),
                    code: workspaceFileSystemErrorCode(cause),
                    cause,
              ...
              Effect.mapError(
                (cause) =>
                  new ProjectCreateFileError({
                    message: workspaceFileOperationMessage({
                      action: "Failed to create workspace file",
                      unsafePathMessage: "Workspace file path must stay within the project root.",
                      cause,
                    }),
                    code: workspaceFileSystemErrorCode(cause),
        ...
        [WS_METHODS.projectsCreateFolder]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsCreateFolder,
            workspaceFileSystem.createFolder(input).pipe(
              Effect.mapError(
                (cause) =>
                  new ProjectCreateFolderError({
                    message: workspaceFileOperationMessage({
                      action: "Failed to create workspace folder",
                      unsafePathMessage: "Workspace folder path must stay within the project root.",
                      cause,
                    }),
                    code: workspaceFileSystemErrorCode(cause),
                    cause,
