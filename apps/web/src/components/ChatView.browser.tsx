...
import { scopedThreadKey, scopeThreadRef } from "@snocode/client-runtime";
import { createModelCapabilities, createModelSelection } from "@snocode/shared/model";
...
import { DEFAULT_CLIENT_SETTINGS } from "@snocode/contracts/settings";
...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.snocode-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
    ...
    availableEditors: [],
    observability: {
      logsDirectoryPath: "/repo/project/.snocode/logs",
      ...
      sourceControlProviders: [
        {
          ...
          auth: {
            ...
            account: Option.some("example-org"),
...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              SNOCODE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.SNOCODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("snocode:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              SNOCODE_PROJECT_ROOT: "/repo/project",
              SNOCODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/snowopsdev/snocode/pull/1359",
              baseBranch: "main",
          ...
          expect(dispatchRequest).toMatchObject({
            ...
            bootstrap: {
              ...
              prepareWorktree: {
                ...
                baseBranch: "main",
                branch: expect.stringMatching(/^snocode\/[0-9a-f]{8}$/),
              },
              runSetupScript: true,
            },
          });
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("snocode:client-settings:v1");
      await mounted.cleanup();
    }
  });
        ...
        threads: createSnapshotForTargetUser({
          ...
          thread.id === THREAD_ID
            ? Object.assign({}, thread, {
                ...
                worktreePath: "/repo/.snocode/worktrees/existing",
              })
            : thread,
        ),
      },
    ...
    const mounted = await mountChatView({
      ...
      resolveRpc: (body) => {
        ...
        if (body._tag === WS_METHODS.sourceControlLookupRepository) {
          return {
            provider: "github",
            nameWithOwner: "example-org/example-env",
            url: "https://github.com/example-org/example-env",
            sshUrl: "git@github.com:example-org/example-env.git",
          };
        }
    ...
    try {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(document.body.textContent).toContain("example-org/example-env");
          expect(document.body.textContent).toContain("https://github.com/example-org/example-env");
      ...
      await vi.waitFor(
        () => {
          ...
          expect(cloneRequest).toMatchObject({
            remoteUrl: "git@github.com:example-org/example-env.git",
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: Snocode Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
