...
import {
  ...
  scopeProjectRef,
  scopeThreadRef,
} from "@snocode/client-runtime";
import {
  applyClaudePromptEffortPrefix,
  createModelSelection,
  resolvePromptInjectedEffort,
} from "@snocode/shared/model";
import { projectScriptCwd, projectScriptRuntimeEnv } from "@snocode/shared/projectScripts";
import { truncate } from "@snocode/shared/String";
...
import { buildTemporaryWorktreeBranchName } from "@snocode/shared/git";
...
import {
  appendBrowserPreviewContextsToPrompt,
  type BrowserPreviewAppendRequest,
  type BrowserPreviewContextDraft,
  previewContextLabel,
...
type ChatViewProps =
  | {
      ...
      browserPreviewAppendRequest?: BrowserPreviewAppendRequest | null;
      routeKind: "server";
      draftId?: never;
    }
  | {
...
export default function ChatView(props: ChatViewProps) {
  ...
  const addComposerDraftBrowserPreviewContext = useComposerDraftStore(
    (store) => store.addBrowserPreviewContext,
  );
  const setComposerDraftBrowserPreviewContexts = useComposerDraftStore(
    (store) => store.setBrowserPreviewContexts,
  );
  ...
  const composerBrowserPreviewContextsRef = useRef<BrowserPreviewContextDraft[]>([]);
  ...
  const handledBrowserPreviewAppendRequestIdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    const request = props.browserPreviewAppendRequest;
    if (!request || handledBrowserPreviewAppendRequestIdRef.current === request.id) {
      return;
    }
    handledBrowserPreviewAppendRequestIdRef.current = request.id;
    addComposerDraftBrowserPreviewContext(composerDraftTarget, {
      ...request,
      createdAt: new Date().toISOString(),
    });
    composerRef.current?.focusAtEnd();
  }, [
    addComposerDraftBrowserPreviewContext,
    composerDraftTarget,
    composerRef,
    props.browserPreviewAppendRequest,
  ]);
  ...
  useEffect(() => {
    if (!gitCwd && workspaceExplorerOpen) {
      setWorkspaceExplorerOpen(false);
    }
  }, [gitCwd, workspaceExplorerOpen]);
  ...
  const onTogglePreview = useCallback(() => {
    ...
    void navigate({
      ...
      search: (previous) => {
        const rest = stripDiffSearchParams(previous);
        return previewOpen ? { ...rest, preview: undefined } : { ...rest, preview: "1" };
      },
    });
  }, [environmentId, isServerThread, navigate, previewOpen, threadId]);
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    const {
      ...
      browserPreviewContexts: composerBrowserPreviewContexts,
    ...
    const {
      ...
      terminalContexts: composerTerminalContexts,
      browserPreviewContextCount: composerBrowserPreviewContexts.length,
    });
    ...
    const standaloneSlashCommand =
      ...
      composerBrowserPreviewContexts.length === 0
    ...
    const composerBrowserPreviewContextsSnapshot = [...composerBrowserPreviewContexts];
    const messageTextForSend = appendBrowserPreviewContextsToPrompt(
      appendTerminalContextsToPrompt(promptForSend, composerTerminalContextsSnapshot),
      composerBrowserPreviewContextsSnapshot,
    );
    ...
    await (async () => {
      ...
      if (!titleSeed) {
        ...
        } else if (composerBrowserPreviewContextsSnapshot.length > 0) {
          titleSeed = previewContextLabel(composerBrowserPreviewContextsSnapshot[0]!.context);
        } else {
      ...
      if (
        ...
        composerBrowserPreviewContextsRef.current.length === 0
      ) {
        setOptimisticUserMessages((existing) => {
        ...
        composerBrowserPreviewContextsRef.current = composerBrowserPreviewContextsSnapshot;
        setComposerDraftPrompt(composerDraftTarget, promptForSend);
        addComposerDraftImages(composerDraftTarget, retryComposerImages);
        setComposerDraftTerminalContexts(composerDraftTarget, composerTerminalContextsSnapshot);
        setComposerDraftBrowserPreviewContexts(
          composerDraftTarget,
          composerBrowserPreviewContextsSnapshot,
        );
        composerRef.current?.resetCursorState({
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      >
        <ChatHeader
          ...
          gitCwd={gitCwd}
          workspaceExplorerAvailable={gitCwd !== null}
      ...
      <div className="flex min-h-0 min-w-0 flex-1">
            ...
            <div className="relative isolate">
              ...
              <div className="relative z-10">
                <ChatComposer
                  ...
                  composerBrowserPreviewContextsRef={composerBrowserPreviewContextsRef}
        ...
        {workspaceExplorerOpen && !shouldUsePlanSidebarSheet && gitCwd ? (
      ...
      {shouldUsePlanSidebarSheet ? (
        <RightPanelSheet open={workspaceExplorerOpen} onClose={closeWorkspaceExplorer}>
          {gitCwd ? (
            <WorkspaceExplorerPanel
              environmentId={environmentId}
              cwd={gitCwd}
