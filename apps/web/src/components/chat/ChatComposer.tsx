...
import type { UnifiedSettings } from "@snocode/contracts/settings";
...
export interface ChatComposerProps {
  ...
  composerBrowserPreviewContextsRef: React.RefObject<BrowserPreviewContextDraft[]>;
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  const {
    ...
    composerBrowserPreviewContextsRef,
    shouldAutoScrollRef,
  ...
  const composerBrowserPreviewContexts = composerDraft.browserPreviewContexts;
  ...
  const removeComposerDraftBrowserPreviewContext = useComposerDraftStore(
    (store) => store.removeBrowserPreviewContext,
  );
  ...
  const composerSendState = useMemo(
    ...
    [
      composerBrowserPreviewContexts.length,
  ...
  const removeComposerBrowserPreviewContext = useCallback(
    (contextId: string) => {
      removeComposerDraftBrowserPreviewContext(composerDraftTarget, contextId);
    },
    [composerDraftTarget, removeComposerDraftBrowserPreviewContext],
  );
  ...
  useEffect(() => {
    composerBrowserPreviewContextsRef.current = composerBrowserPreviewContexts;
  }, [composerBrowserPreviewContexts, composerBrowserPreviewContextsRef]);
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      getSendContext: () => ({
        ...
        browserPreviewContexts: composerBrowserPreviewContextsRef.current,
          ...
          >
            ...
            {!isComposerCollapsedMobile &&
              ...
              composerBrowserPreviewContexts.length > 0 && (
                <div className="mb-3 flex flex-wrap gap-2">
                  {composerBrowserPreviewContexts.map((entry) => {
                    const label = previewContextLabel(entry.context);
                    const selector = entry.context.selectorCandidates[0] ?? entry.context.domPath;
                    const tooltipText = [
                      label,
                      selector,
                      entry.mode === "change-template" ? "Change request template" : "DOM context",
                    ].join("\n");
                    return (
                      <Tooltip key={entry.id}>
                        <TooltipTrigger
                          render={
                            <div className="group inline-flex max-w-full items-center gap-2 rounded-md border border-border/80 bg-background px-2.5 py-1.5 text-xs shadow-sm">
                              <ScanLineIcon className="size-3.5 shrink-0 text-muted-foreground" />
                              <span className="min-w-0 max-w-56 truncate font-medium">{label}</span>
                              ...
                              <Button
                                variant="ghost"
                                size="icon-xs"
                                className="-mr-1 size-5 shrink-0 opacity-70 hover:opacity-100"
                                onClick={() => removeComposerBrowserPreviewContext(entry.id)}
