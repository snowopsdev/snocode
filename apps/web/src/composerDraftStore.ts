...
import { createModelSelection, normalizeModelSlug } from "@snocode/shared/model";
...
import type { BrowserPreviewContextDraft } from "./lib/browserPreviewContext";
import { create } from "zustand";
import { createJSONStorage, persist } from "zustand/middleware";
import { useShallow } from "zustand/react/shallow";
import { createDebouncedStorage, createMemoryStorage } from "./lib/storage";
import { getDefaultServerModel } from "./providerModels";
import { UnifiedSettings } from "@snocode/contracts/settings";
const isRuntimeMode = Schema.is(RuntimeMode);
const isProviderDriverKind = Schema.is(ProviderDriverKind);

export const COMPOSER_DRAFT_STORAGE_KEY = "snocode:composer-drafts:v1";
...
export interface ComposerThreadDraftState {
  ...
  browserPreviewContexts: BrowserPreviewContextDraft[];
  /**
...
interface ComposerDraftStoreState {
  ...
  setBrowserPreviewContexts: (
    threadRef: ComposerThreadTarget,
    contexts: BrowserPreviewContextDraft[],
  ) => void;
  setModelSelection: (
    threadRef: ComposerThreadTarget,
  ...
  addBrowserPreviewContext: (
    threadRef: ComposerThreadTarget,
    context: BrowserPreviewContextDraft,
  ) => void;
  removeBrowserPreviewContext: (threadRef: ComposerThreadTarget, contextId: string) => void;
  clearBrowserPreviewContexts: (threadRef: ComposerThreadTarget) => void;
...
const EMPTY_BROWSER_PREVIEW_CONTEXTS: BrowserPreviewContextDraft[] = [];
Object.freeze(EMPTY_IMAGES);
Object.freeze(EMPTY_IDS);
Object.freeze(EMPTY_PERSISTED_ATTACHMENTS);
Object.freeze(EMPTY_BROWSER_PREVIEW_CONTEXTS);
...
const EMPTY_THREAD_DRAFT = Object.freeze<ComposerThreadDraftState>({
  ...
  browserPreviewContexts: EMPTY_BROWSER_PREVIEW_CONTEXTS,
  modelSelectionByProvider: EMPTY_MODEL_SELECTION_BY_PROVIDER,
  activeProvider: null,
  runtimeMode: null,
...
function browserPreviewContextDedupKey(context: BrowserPreviewContextDraft): string {
  const selected = context.context;
  return [
    selected.pageUrl,
    selected.tagName,
    selected.selectorCandidates[0] ?? selected.domPath,
    selected.accessibleName,
    selected.text,
  ].join("\u0000");
}

function dedupeBrowserPreviewContexts(
  contexts: ReadonlyArray<BrowserPreviewContextDraft>,
): BrowserPreviewContextDraft[] {
  const existingIds = new Set<string>();
  const existingDedupKeys = new Set<string>();
  const dedupedContexts: BrowserPreviewContextDraft[] = [];
  ...
  for (const context of contexts) {
    ...
    const dedupKey = browserPreviewContextDedupKey(context);
    if (existingIds.has(id) || existingDedupKeys.has(dedupKey)) {
      continue;
    }
    dedupedContexts.push({ ...context, id });
    existingIds.add(id);
...
function shouldRemoveDraft(draft: ComposerThreadDraftState): boolean {
  return (
    ...
    draft.browserPreviewContexts.length === 0 &&
  ...
  persist(
    (setBase, get) => {
      ...
      return {
        ...
        setBrowserPreviewContexts: (threadRef, contexts) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            ...
            const nextDraft: ComposerThreadDraftState = {
              ...existing,
              browserPreviewContexts: dedupeBrowserPreviewContexts(contexts),
            };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadKey];
        ...
        addBrowserPreviewContext: (threadRef, context) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const browserPreviewContexts = dedupeBrowserPreviewContexts([
              ...existing.browserPreviewContexts,
              context,
            ]);
            if (browserPreviewContexts.length === existing.browserPreviewContexts.length) {
              return state;
            }
            return {
              draftsByThreadKey: {
        ...
        removeBrowserPreviewContext: (threadRef, contextId) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0 || contextId.length === 0) {
            return;
          }
          set((state) => {
            ...
            const nextDraft: ComposerThreadDraftState = {
              ...current,
              browserPreviewContexts: current.browserPreviewContexts.filter(
                (context) => context.id !== contextId,
              ),
            };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
        ...
        clearBrowserPreviewContexts: (threadRef) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current || current.browserPreviewContexts.length === 0) {
              return state;
            }
            const nextDraft: ComposerThreadDraftState = {
              ...current,
