import { EnvironmentId, type VcsStatusResult } from "@snocode/contracts";
...
function createRegisteredGitStatusClient(environmentId: EnvironmentId) {
  ...
  const client = {
    ...
    projects: {
      ...
      listDirectory: vi.fn(async () => ({ relativePath: null, entries: [] })),
      readFile: vi.fn(async () => ({ relativePath: "README.md", contents: "" })),
      createFile: vi.fn(async () => ({ relativePath: "README.md" })),
      createFolder: vi.fn(async () => ({ relativePath: "docs" })),
