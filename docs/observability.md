...
Snocode has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.snocode/userdata/logs/server.trace.ndjson`).
...
```bash
export SNOCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export SNOCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export SNOCODE_OTLP_SERVICE_NAME=snocode-local
```

Optional:

```bash
export SNOCODE_TRACE_MIN_LEVEL=Info
export SNOCODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `SNOCODE_OTLP_*`.

macOS app bundle example:

```bash
SNOCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
SNOCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
SNOCODE_OTLP_SERVICE_NAME=snocode-desktop \
"/Applications/Snocode.app/Contents/MacOS/Snocode"
```
...
```bash
tail -f "$SNOCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$SNOCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$SNOCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `snocode-local`, `snocode-dev`, or `snocode-desktop`
...
- `SNOCODE_OTLP_TRACES_URL` was not set
...
- `SNOCODE_TRACE_FILE`: override trace file path
- `SNOCODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `SNOCODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `SNOCODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `SNOCODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `SNOCODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `SNOCODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `SNOCODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `SNOCODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `SNOCODE_OTLP_SERVICE_NAME`: service name, default `snocode-server`
