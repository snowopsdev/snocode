...
This guide is for people who want to use more than one Codex account in Snocode.
...
- both accounts can see the same Snocode/Codex sessions
...
In Snocode Settings, name it something obvious:
...
In Snocode Settings, add another Codex provider:
...
Snocode shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Snocode treats it
