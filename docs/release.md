...
- Publishes the CLI package (`apps/server`, npm package `snocode`) with OIDC trusted publishing from the same workflow file:
...
- `SNOCODE_WEB_ROUTER_URL`: defaults to `https://app.snowcode.io`.
- `SNOCODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.snowcode.io`.
- `SNOCODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.snowcode.io`.

Required Vercel domains:

- `app.snowcode.io`: the router domain users open, updated by stable releases.
- `latest.app.snowcode.io`: channel alias updated by stable releases.
- `nightly.app.snowcode.io`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__snocode/channel?channel=latest` or
`/__snocode/channel?channel=nightly`; the router stores the
`snocode_web_channel` cookie and rewrites future requests on `app.snowcode.io` to
...
through `/__snocode/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.snowcode.io` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `snocode`) to the `nightly` npm dist-tag using the same nightly version.
...
- Repository slug source:
  - `SNOCODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `SNOCODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `snocode` (or rename package first if needed).
