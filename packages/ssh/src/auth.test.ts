...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "snocode-ssh-askpass-test-" });
      ...
      assert.equal(env.SNOCODE_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "snocode");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\snocode-ssh-askpass",
        platform: "win32",
      }).pipe(Effect.provide(NodeServices.layer));

      assert.equal(descriptor.launcherPath, "C:\\temp\\snocode-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
