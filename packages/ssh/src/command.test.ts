...
import {
  ...
  resolveRemoteSnocodeCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote snocode package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteSnocodeCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "snocode@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteSnocodeCliPackageSpec({
