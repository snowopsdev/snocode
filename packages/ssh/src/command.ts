...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@snocode/contracts";
...
const PUBLISHABLE_SNOCODE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteSnocodeCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_SNOCODE_VERSION_PATTERN.test(appVersion)) {
    return `snocode@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "snocode@nightly";
  }

  return input.updateChannel === "nightly" ? "snocode@nightly" : "snocode@latest";
}
