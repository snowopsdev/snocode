...
import {
  ...
  buildRemoteSnocodeRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote snocode runner with npx and npm fallbacks", () => {
    const script = buildRemoteSnocodeRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "SNOCODE_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec snocode "$@"');
    assert.include(script, "exec npx --yes 'snocode@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'snocode@latest' -- \"$@\"");
    assert.include(script, "could not install 'snocode@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `SNOCODE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for SNOCODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteSnocodeRunnerScript();

    assert.include(script, "SNOCODE_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote snocode runner", () => {
    const script = buildRemoteSnocodeRunnerScript({
      packageSpec: "snocode@nightly; touch /tmp/snocode-owned",
    });

    assert.include(script, "exec npx --yes 'snocode@nightly; touch /tmp/snocode-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'snocode@nightly; touch /tmp/snocode-owned' -- \"$@\"",
    );
    assert.notInclude(script, "exec npx --yes snocode@nightly; touch /tmp/snocode-owned");
  });

  it("builds the remote snocode runner with a node script override", () => {
    const script = buildRemoteSnocodeRunnerScript({
    ...
    assert.include(
      script,
      "SNOCODE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$SNOCODE_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote snocode runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `SNOCODE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Snocode server did not become ready");
    assert.include(buildRemoteLaunchScript({ packageSpec: "snocode@nightly" }), "snocode@nightly");
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "snocode@nightly" }),
      "snocode@nightly",
    );
    assert.include(
