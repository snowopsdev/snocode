...
import * as NetService from "@snocode/shared/Net";
import { extractJsonObject, fromLenientJson } from "@snocode/shared/schemaJson";
import { satisfiesSemverRange } from "@snocode/shared/semver";
...
export interface RemoteSnocodeRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteSnocodeRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteSnocodeRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  SNOCODE_NODE_ENGINE_RANGE=@@SNOCODE_NODE_ENGINE_RANGE@@
  if [ -z "$SNOCODE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$SNOCODE_NODE_ENGINE_RANGE" <<'NODE'
@@SNOCODE_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for SNOCODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$SNOCODE_NODE_BIN/node" ]; then
        PATH="$SNOCODE_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@SNOCODE_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
SNOCODE_NODE_SCRIPT_PATH=@@SNOCODE_NODE_SCRIPT_PATH@@
if [ -n "$SNOCODE_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$SNOCODE_NODE_SCRIPT_PATH" "$@"
fi
if command -v snocode >/dev/null 2>&1; then
  exec snocode "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@SNOCODE_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@SNOCODE_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the snocode CLI and could not install @@SNOCODE_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@SNOCODE_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.snocode/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.snocode"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-snocode.sh"
RUNNER_NEXT="$STATE_DIR/run-snocode.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@SNOCODE_DEFAULT_REMOTE_PORT@@" "@@SNOCODE_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@SNOCODE_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@SNOCODE_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@SNOCODE_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@SNOCODE_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@SNOCODE_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@SNOCODE_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env SNOCODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@SNOCODE_READY_TIMEOUT_MS@@"; then
    printf 'Remote Snocode server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.snocode/ssh-launch/@@SNOCODE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.snocode"
RUNNER_FILE="$STATE_DIR/run-snocode.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteSnocodeRunnerScript(input?: RemoteSnocodeRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "snocode@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      SNOCODE_PACKAGE_SPEC: packageSpec,
      SNOCODE_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      SNOCODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

function buildRemoteNodeEnvScript(input?: RemoteSnocodeRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      SNOCODE_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      SNOCODE_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteSnocodeRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    SNOCODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    SNOCODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteSnocodeRunnerScript(input)),
    SNOCODE_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    SNOCODE_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    SNOCODE_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    SNOCODE_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    SNOCODE_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    SNOCODE_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    SNOCODE_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteSnocodeRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    SNOCODE_STATE_KEY: remoteStateKey(target),
    SNOCODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteSnocodeRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteSnocodeRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteSnocodeRunnerOptions;
...
>()("@snocode/ssh/SshEnvironmentManager") {
