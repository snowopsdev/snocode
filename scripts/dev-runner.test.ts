...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit SNOCODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "SNOCODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid SNOCODE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults SNOCODE_HOME to ~/.snocode when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SNOCODE_HOME, path.resolve(NodeOS.homedir(), ".snocode"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          snocodeHome: "/tmp/custom-snocode",
          noBrowser: true,
        ...
        assert.equal(env.SNOCODE_HOME, path.resolve("/tmp/custom-snocode"));
        assert.equal(env.SNOCODE_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.SNOCODE_NO_BROWSER, "1");
        assert.equal(env.SNOCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.SNOCODE_LOG_WS_EVENTS, "1");
        assert.equal(env.SNOCODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            SNOCODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          snocodeHome: undefined,
        ...
        assert.equal(env.SNOCODE_MODE, "web");
        assert.equal(env.SNOCODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SNOCODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom snocodeHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.SNOCODE_HOME, path.resolve("/tmp/my-snocode"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            SNOCODE_PORT: "13773",
            SNOCODE_MODE: "web",
            SNOCODE_NO_BROWSER: "0",
            SNOCODE_HOST: "0.0.0.0",
        ...
        assert.equal(env.SNOCODE_MODE, undefined);
        assert.equal(env.SNOCODE_NO_BROWSER, undefined);
        assert.equal(env.SNOCODE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SNOCODE_PORT, "13773");
