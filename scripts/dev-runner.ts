...
export const DEFAULT_SNOCODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".snocode"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@snocode/contracts",
    "--filter=@snocode/web",
    "--filter=snocode",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=snocode"],
  "dev:web": ["run", "dev", "--filter=@snocode/web"],
  "dev:desktop": ["run", "dev", "--filter=@snocode/desktop", "--filter=@snocode/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("SNOCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("SNOCODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid SNOCODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `SNOCODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric SNOCODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed SNOCODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_SNOCODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly snocodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(snocodeHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      SNOCODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.SNOCODE_PORT = String(serverPort);
      ...
      delete output.SNOCODE_MODE;
      delete output.SNOCODE_NO_BROWSER;
      delete output.SNOCODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.SNOCODE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.SNOCODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.SNOCODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.SNOCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.SNOCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.SNOCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.SNOCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.SNOCODE_MODE = "web";
      delete output.SNOCODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read SNOCODE_PORT_OFFSET/SNOCODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.SNOCODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.SNOCODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  snocodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Snocode data (equivalent to SNOCODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("SNOCODE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to SNOCODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("SNOCODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to SNOCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("SNOCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to SNOCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("SNOCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to SNOCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("SNOCODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to SNOCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("SNOCODE_PORT")),
  ),
